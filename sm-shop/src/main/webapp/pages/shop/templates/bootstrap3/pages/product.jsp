            ...
            <div id="shop" class="container">
                ...
                <div itemscope class="col-sm-12 col-md-12 mainProductColumn" itemtype="http://data-vocabulary.org/Product">
						...
						<div id="img" class="col-sm-6 col-md-6 productMainImage">
							...
							<span id="mainImg"><img id="im-<c:out value="${product.image.id}"/>" alt="<c:out value="${product.description.name}"/>" src="${product.image.imageUrl}" data-zoom-image="<sm:shopProductImage imageName="${product.image.imageName}" sku="${product.sku}" size="LARGE"/>"></span>												
							<script>
								$(function() {
							...
							<c:if test="${product.images!=null && fn:length(product.images)>1}">
								<div id="imageGallery" class="row">
									...
									<div class="col-xs-6 col-md-3">
										<a href="#img" class="thumbImg thumbnail" title="<c:out value="${thumbnail.imageName}"/>"><img id="im-<c:out value="${thumbnail.id}"/>" src="${thumbnail.imageUrl}" data-zoom-image="<sm:shopProductImage imageName="${thumbnail.imageName}" sku="${product.sku}" size="LARGE"/>" alt="<c:url value="${thumbnail.imageName}"/>" ></a>
									</div>
									</c:forEach>								
								</div>
							</c:if>
