...
public class ShoppingOrderController extends AbstractController {
	...
	public String displayCheckout(@CookieValue("cart") String cookie, Model model, HttpServletRequest request,
		...
		if (StringUtils.isBlank(shoppingCartCode)) {
			...
			if (cookie == null) {// session expired and cookie null, nothing to
									// do
		...
		if (requiresShipping) {
			...
			setSessionAttribute(Constants.SHIPPING_QUOTE, quote, request);
		}
	...
	private Order commitOrder(ShopOrder order, HttpServletRequest request, Locale locale)
		...
		super.removeAttribute(Constants.SHIPPING_QUOTE, request);
	...
	public String commitOrder(@CookieValue("cart") String cookie,
		...
		try {
			...
			orderFacade.validateOrder(order, bindingResult, shippingQoute, new HashMap<String, String>(), store,
					locale);
	...
	public @ResponseBody ReadableShopOrder calculateShipping(@ModelAttribute(value = "order") ShopOrder order,
		 ...
		 * String jsonInString = mapper.writeValueAsString(order);
		 * LOGGER.info("Calculate order -> shoppingCartCode[ " +
		 * shoppingCartCode + "] -> " + jsonInString); } catch(Exception de) {
		 * LOGGER.error(de.getMessage()); } } }
		 */
		...
		try {
			...
			if (quote != null) {
				...
				if (CollectionUtils.isNotEmpty(quote.getShippingOptions())
					 ...
					 * LOGGER.error("Cannot cast value to string " +
					 * e.getMessage()); } } }
					 */
					...
					readableOrder.setShippingSummary(readableSummary);// TODO
																		...
																		// readable
																		// address
					setSessionAttribute(Constants.SHIPPING_SUMMARY, summary, request);
					setSessionAttribute(Constants.SHIPPING_OPTIONS, options, request);
					setSessionAttribute("SHIPPING_INFORMATIONS", readableSummary.getQuoteInformations(), request);
					setSessionAttribute(Constants.SHIPPING_QUOTE, quote, request);
