...
public class OrderFacadeImpl implements OrderFacade {
	...
	private Order processOrderModel(ShopOrder order, Customer customer, Transaction transaction, MerchantStore store,
		...
		try {
			...
			if (PaymentType.INSTAMOJO.name().equals(paymentType)) {
				...
				((InstamojoPayment) payment).setTransactionId(transaction.getTransactionDetails().get(
						com.salesmanager.core.business.modules.integration.payment.impl.InstamojoPayment.TRANSACTION_ID));
			}
	...
	@Override
	public void validateOrder(ShopOrder order, BindingResult bindingResult, ShippingQuote shippingQoute,
			Map<String, String> messagesResult, MerchantStore store, Locale locale) throws ServiceException {
		...
		try {
			...
			if (shippingService.requiresShipping(order.getShoppingCartItems(), store)
					&& order.getSelectedShippingOption() == null && !shippingQoute.isFreeShipping()) {
				ServiceException serviceException = new ServiceException(ServiceException.EXCEPTION_VALIDATION,
						"shipping.required");
				throw serviceException;
			}
